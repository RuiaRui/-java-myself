...
public class EmployeeArray {
    ...
    public static Employee[] makeArray(Employee first, Employee second,
                                       Employee third) {
        Employee[] employeeList = {first,second,third};

        return employeeList; // REMOVE; USED SO THIS FILE COMPILES
    }
    ...
    public static double getHighestSalary(Employee[] array) {
        ...
        for(int i=0;i<array.length;i++){
            if(hsalary<array[i].getSalary()){
                hsalary=array[i].getSalary();
            }
        }

        return hsalary; // REMOVE; USED SO THIS FILE COMPILES
    }
