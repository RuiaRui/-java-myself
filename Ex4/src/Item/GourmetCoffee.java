...
public class GourmetCoffee  {
    ...
    /**
     * Constructs a <code>Item.GourmetCoffee</code> object and
    ...
    private Catalog loadCatalog() {
        ...
        catalog.addProduct(new CoffeeBrewer(
                "B001", "Home Item.Coffee Brewer", 150.00,
                "Brewer 100", "Pourover", 6));
        catalog.addProduct(new CoffeeBrewer(
                "B002", "Item.Coffee Brewer, 2 Warmers", 200.00,
                "Brewer 200", "Pourover", 12));
        catalog.addProduct(new CoffeeBrewer(
                "B003", "Item.Coffee Brewer, 3 Warmers", 280.00,
        ...
        catalog.addProduct(
                new Product("A005", "Gourmet Item.Coffee Cookies", 12.00));
    ...
    public void displayCatalog() {
        ...
        if (size == 0) {
            ...
            for (Product product :this.catalog.getProducts()) {
    ...
    public void displayOrdersSold() {
        ...
        if (numOrders != 0) {
            int orderNumber = 1;
            for (Order  order : this.sales.getOrders()) {
     ...
     *
     * @param product the <code>Item.Product</code> object to be displayed.
     */
    public void displayNumberOfOrders(Product product) {
        ...
        stdOut.println("the number of items is " +count);
    ...
    public void displayTotalQuantityOfProducts() {
        int [] tolQuantity =new int[catalog.getNumberOfProducts()];
        for (Order order : this.sales.getOrders()) {
            for (OrderItem orderitem : order.getItems()) {
                for (int k=0;k<catalog.getNumberOfProducts();k++) {
                    if(orderitem.getProduct().equals(catalog.getProducts().get(k))){
                        tolQuantity[k]+=orderitem.getQuantity();
        ...
        for(int i = 0;i <= catalog.getNumberOfProducts() - 1;i++){
            stdErr.println("the quantity of  "+catalog.getProducts().get(i).getCode()+" is  "+tolQuantity[i]);
        }
    }

    /*
     * Prompts user for a product code and locates the associated
     * <code>Item.Product</code> object.
     *
     * @return reference to the <code>Item.Product</code> object with
     *         the specified code
     */
    private Product readProduct() throws IOException  {

        do  {
            stdErr.print("Item.Product code> ");
            stdErr.flush();
